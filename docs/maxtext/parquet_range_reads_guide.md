...
```bash
cd workloads/maxtext-dataset-loader/helm_chart
```
...
### Benchmark Run D: MaxText In-Tree Native DataLoader
```bash
helm install maxtext-in-tree-run . \
  --set workload.loaderMode="in_tree_loader" \
  --set workload.inTreeLoader.datasetType="synthetic" \
  --set workload.inTreeLoader.hardware="cpu" \
  --set workload.batchSize=4 \
  --set workload.maxBatches=100
```
