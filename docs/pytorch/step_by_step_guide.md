...
Workloads can be executed either **directly via Helm** (without CloudBuild) or via **automated CloudBuild pipelines**.
...
You can deploy PyTorch benchmark jobs across three different storage backends (**Google Cloud Managed Lustre**, **GCSFuse**, and **`gcsfs`**). Each backend can be deployed either **directly via Helm** (no CloudBuild needed) or **via CloudBuild**.

---

### Option A: Direct Helm Deployment (No CloudBuild Required)
...
### Option B: Automated CloudBuild Pipeline

Alternatively, submit benchmark jobs through CloudBuild automated execution scripts:
...
gcloud builds submit \
  ...
  --substitutions=\
_USE_LUSTRE="true",\
_LUSTRE_INSTANCE="${LUSTRE_INSTANCE}",\
_NODES="2",\
_RANKS_PER_NODE="4",\
_STEPS="100",\
