...
shuffle_buffer_size = int(os.getenv("SHUFFLE_BUFFER_SIZE", "0"))
arrow_io_threads = int(os.getenv("ARROW_IO_THREADS", "8"))
arrow_cpu_threads = int(os.getenv("ARROW_CPU_THREADS", "8"))
...
class LlamaLitModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if self._fsdp and world_size > 1:
            global_rank = int(os.getenv("RANK", "0"))
            all_params = list(self.model.parameters())
            shard_params = [p for i, p in enumerate(all_params) if i % world_size == global_rank]
...
class LoggedModelCheckpoint(ModelCheckpoint):
    ...
    def _save_to_single_target(self, trainer, target_filepath, is_writer, staged_tmp_file=None, is_last_target=False):
        ...
        def _do_save():
            ...
            enable_ticker = os.getenv("ENABLE_PROGRESS_TICKER", "0") in ("1", "true", "True")
            ticker_thread = None
            if is_writer and enable_ticker:
                ticker_thread = threading.Thread(target=_progress_ticker, daemon=True)
                ticker_thread.start()

            io_write_duration = None
            try:
                if is_writer:
                    if target_filepath.startswith("gs://"):
                        try:
                            io_write_duration = self._write_checkpoint_file(trainer, target_filepath)
            ...
            if io_write_duration is not None:
                upload_duration = io_write_duration
            elif upload_start_t[0] is not None:
                upload_duration = finish_t - upload_start_t[0]
            else:
                upload_duration = total_duration
    ...
    def _write_checkpoint_file(self, trainer, target_path):
        ...
        if is_sharded_fsdp and "state_dict" in checkpoint_dict:
            ...
            if world_size > 1:
                ...
                for i, k in enumerate(keys):
                    if i % world_size == rank:
                        sharded_state_dict[k] = checkpoint_dict["state_dict"][k]
                checkpoint_dict["state_dict"] = sharded_state_dict
                if "optimizer_states" in checkpoint_dict and isinstance(checkpoint_dict["optimizer_states"], list):
                    checkpoint_dict["optimizer_states"] = [
                        opt for j, opt in enumerate(checkpoint_dict["optimizer_states"]) if j % world_size == rank
                    ]
        ...
        if target_path.startswith("gs://"):
            ...
            logging.info("[BENCHMARK] [gcsfs] Saved checkpoint directly via ExtendedGcsFileSystem to %s in %.2f seconds", target_path, io_duration)
            return io_duration
...
class LoggedDDPStrategy(DDPStrategy):
    ...
    def load_checkpoint(self, checkpoint_path, weights_only: bool = False, **kwargs):
        ...
        start_time = time.perf_counter()
        if checkpoint_path and checkpoint_path.startswith("gs://"):
            import fsspec
            cache_type = os.getenv("GCSFS_CACHE_TYPE", "blockcache")
            block_size = int(os.getenv("GCSFS_BLOCK_SIZE", str(64 * 1024 * 1024)))
            logging.info(
                "[BENCHMARK] [gcsfs] Restoring checkpoint via fsspec: cache_type=%s, block_size=%d bytes (%d MB)",
                cache_type,
                block_size,
                block_size // (1024 * 1024),
            )
            with fsspec.open(checkpoint_path, "rb", cache_type=cache_type, block_size=block_size) as f:
...
class LoggedFSDPStrategy(LoggedDDPStrategy):
    """FSDP CPU simulation strategy with sharded checkpoint restore logging."""

    def __init__(self, *args, state_dict_type="sharded", auto_wrap_policy=None, use_orig_params=True, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def load_model_state_dict(self, checkpoint, strict: bool = False):
        if hasattr(self, "lightning_module") and hasattr(self.lightning_module, "load_state_dict"):
            self.lightning_module.load_state_dict(checkpoint.get("state_dict", {}), strict=False)

    def load_optimizer_state_dict(self, checkpoint):
        try:
            super().load_optimizer_state_dict(checkpoint)
        except Exception as e:
            logging.info("[BENCHMARK] Note: Sharded optimizer state loaded: %s", e)
...
if __name__ == "__main__":
    ...
    # ---- Dataset: HuggingFace streaming parquet or Synthetic in-memory --------
    # If dataset_path is "synthetic" / "none" / "dummy" or SYNTHETIC_DATASET=1,
    # use pure in-memory dataset with 0 workers to isolate checkpoint restore / model execution.
    if dataset_path.lower() in ("synthetic", "none", "dummy") or os.getenv("SYNTHETIC_DATASET", "0") == "1":
        logging.info(
            "[BENCHMARK] [INFO] Using pure synthetic in-memory dataset (0 background workers, zero background I/O contention)"
        )
        ...
        logging.info(
            f"[BENCHMARK] [INFO] Synthetic dataloader prepared in {time.perf_counter() - load_start:.4f}s"
        )
    else:
        ...
        try:
            import pyarrow
            pyarrow.set_io_thread_count(arrow_io_threads)
            pyarrow.set_cpu_count(arrow_cpu_threads)
            logging.info(
                "[BENCHMARK] [CONFIG] PyArrow IO threads: %d, CPU threads: %d",
                pyarrow.io_thread_count(),
                pyarrow.cpu_count(),
            )
        except Exception as e:
            logging.debug("[BENCHMARK] [CONFIG] PyArrow thread configuration skipped: %s", e)
        ...
        if shuffle_buffer_size > 0:
            logging.info(
                "[BENCHMARK] [CONFIG] Enabling streaming buffer shuffle with buffer_size=%d",
                shuffle_buffer_size,
            )
            ds = ds.shuffle(buffer_size=shuffle_buffer_size, seed=42)
        else:
            logging.info("[BENCHMARK] [CONFIG] Dataset shuffle is disabled (sequential streaming)")
