#!/usr/bin/env python3
"""
MaxText Dataset Loading Benchmark & Demo (Parquet & ArrayRecord).

Simulates the MaxText LLM training data pipeline reading multi-file Parquet datasets
via GCS Range Reads and pre-tokenized ArrayRecord streaming. Supports both:
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] [MAXTEXT_DATASET] %(message)s",
    handlers=[logging.StreamHandler(sys.stdout)],
)


def parse_args():
    parser = argparse.ArgumentParser(description="MaxText Dataset Loading Benchmark (Parquet & ArrayRecord)")
    parser.add_argument(
        "--dataset-path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of concurrent shard streams for multi-stream interleaved shuffling (default: 4)",
    )
    parser.add_argument(
        "--rank",
        type=int,
...
def setup_filesystem(dataset_path, access_mode):
    ...
    if access_mode == "gcsfuse" and dataset_path.startswith("gs://"):
        clean_uri = dataset_path.replace("gs://", "")
        parts = clean_uri.split("/", 1)
        prefix = parts[1] if len(parts) > 1 else ""
        dataset_path = f"/gcs/dataset/{prefix}".rstrip("/") if prefix else "/gcs/dataset"
...
# -----------------------------------------------------------------------------
def run_maxtext_dataset_benchmark(dataset_path, access_mode, columns_to_read, batch_size, max_batches, num_threads, rank, world_size, shuffle_mode="none", data_format="parquet", num_streams=4):
    try:
    ...
    logging.info(
        ...
        f"(shuffle_mode={shuffle_mode}, batch_size={batch_size}, max_batches={max_batches}, num_streams={num_streams})..."
    )
    ...
    if data_format == "parquet":
        if shuffle_mode == "two_stage" and num_streams > 1:
            import queue
            import threading
            from concurrent.futures import ThreadPoolExecutor

            actual_streams = min(num_streams, len(rank_files))
            logging.info(
                f"[MAXTEXT] [SHUFFLE: INTERLEAVED] Initializing {actual_streams} concurrent Parquet shard streams..."
            )

            interleave_queue = queue.Queue(maxsize=max(32, actual_streams * 8))
            ...
            def stream_reader_worker(worker_id):
                    ...
                    try:
                        ...
                        for rg_idx in range(parquet_file.num_row_groups):
                            ...
                            for offset in range(0, num_rows, batch_size):
                                if stop_workers.is_set():
                                    break
                                sub_table = table.slice(offset, min(batch_size, num_rows - offset))
                                batch_dict = {col: sub_table[col].to_numpy() for col in sub_table.column_names}
                                batch_bytes = sum(arr.nbytes for arr in batch_dict.values())
                                while not stop_workers.is_set():
                                    try:
                                        interleave_queue.put((batch_dict, len(sub_table), batch_bytes, rg_duration / max(1, math.ceil(num_rows / batch_size)), fpath), timeout=0.1)
                                        break
                                    except queue.Full:
                                        continue
            ...
            with ThreadPoolExecutor(max_workers=actual_streams) as pool:
                futures = [pool.submit(stream_reader_worker, i) for i in range(actual_streams)]
                
                shuffle_buffer = []
                target_buffer_size = min(max(8, actual_streams * 2), max_batches)

                while loaded_batches < max_batches:
                    while len(shuffle_buffer) < target_buffer_size:
                        try:
                            item = interleave_queue.get(timeout=0.05)
                            shuffle_buffer.append(item)
                        except queue.Empty:
                            if not any(f.running() for f in futures) and interleave_queue.empty():
                                break
                            break
                    ...
                    if not shuffle_buffer:
                        if not any(f.running() for f in futures) and interleave_queue.empty():
                            break
                        try:
                            item = interleave_queue.get(timeout=0.5)
                            shuffle_buffer.append(item)
                        except queue.Empty:
                            continue
                    ...
                    batch_dict, num_samples, batch_bytes, rg_duration, src_fpath = shuffle_buffer.pop(pick_idx)
                    ...
                    if loaded_batches % 100 == 0:
                        logging.info(f"  [MaxText Batch {loaded_batches}/{max_batches}] Read {num_samples} samples ({batch_bytes / (1024 * 1024):.2f} MB) [Interleaved {actual_streams} streams]")

                stop_workers.set()
                while not interleave_queue.empty():
                    try:
                        interleave_queue.get_nowait()
                    except queue.Empty:
                        break
        else:
            for fpath in rank_files:
                ...
                for rg_idx in range(parquet_file.num_row_groups):
                    ...
                    for offset in range(0, num_rows, batch_size):
                        ...
                        batch_dict = {col: sub_table[col].to_numpy() for col in sub_table.column_names}
                        batch_bytes = sum(arr.nbytes for arr in batch_dict.values())
                        step_dur = rg_duration / max(1, math.ceil(num_rows / batch_size))

                        if shuffle_mode == "two_stage":
                            shuffle_buffer.append((batch_dict, len(sub_table), batch_bytes, step_dur))
                            if len(shuffle_buffer) < 4 and loaded_batches + len(shuffle_buffer) < max_batches:
                                continue
                            import random
                            idx = random.randint(0, len(shuffle_buffer) - 1)
                            batch_dict, num_samples, batch_bytes, step_dur = shuffle_buffer.pop(idx)
                        else:
                            num_samples = len(sub_table)
                        ...
                        total_feature_bytes += batch_bytes
                        loaded_batches += 1
                        batch_durations.append(step_dur)
        ...
        for fpath in rank_files:
            ...
            if access_mode == "native_gcs":
                import tempfile
                local_fpath = os.path.join(tempfile.gettempdir(), os.path.basename(fpath))
                import gcsfs
