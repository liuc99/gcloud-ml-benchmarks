...
description: Sub-skill for constructing dynamic Helm chart flags, deploying benchmark releases, monitoring Pod states asynchronously, and handling emergency teardown if cancelled midway.
---
...
Navigate to `workloads/hf-pytorch-lightning-cpu/helm_chart` (or `workloads/tensorstore-gcsfuse/helm_chart`) and execute `helm install`.
